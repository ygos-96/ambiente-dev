import"./index-BI9FPup0.js";import{S as Ye,L as Ue,G as Ve}from"./Sidebar-B88xaBQr.js";const Oe="/ambiente-dev/";document.addEventListener("DOMContentLoaded",async()=>{if(!(sessionStorage.getItem("authenticated")==="true")){window.location.href=`${Oe}index.html`;return}const Q=sessionStorage.getItem("username")||"Usuário",R=new Ye(Q,"home"),ke=document.getElementById("app");ke.innerHTML=`
    ${R.render()}
  ...
  `;const W=document.getElementById("loading-state"),Se=document.getElementById("error-state"),Be=document.getElementById("content"),Le=document.getElementById("next-events-carousel"),Y=document.getElementById("all-events-carousel");document.getElementById("logout-btn");const T=document.getElementById("event-view-modal"),M=document.getElementById("all-events-modal"),_=document.getElementById("all-events-modal-list"),Ce=document.getElementById("carousel-track"),K=document.getElementById("carousel-prev"),X=document.getElementById("carousel-next"),Ie=document.querySelectorAll(".carousel-dot"),E=document.getElementById("create-event-modal"),Z=document.getElementById("open-create-event-modal"),ee=document.getElementById("close-create-event-modal"),te=document.getElementById("cancel-create-event-btn"),H=document.getElementById("create-event-form"),oe=document.getElementById("create-event-date"),se="https://script.google.com/macros/s/AKfycbwurPvvvkvydBx2qN4WgZKMwsvXm31w3wMl_Qx9C70PAUT0hiUz-yHfaJgez2wrb7BTlQ/exec";let w;try{const e=new Ue({storageKey:"calendar-events"});if(se)try{const t=new Ve({scriptUrl:se});await t.initialize(),w=t}catch(t){console.error("❌ Erro ao conectar Google Apps Script:",t),w=e}await w.initialize()}catch(e){console.error("❌ Erro ao inicializar armazenamento:",e),ve("Erro ao conectar com o armazenamento de dados");return}const U=e=>{const t=new Date(e+"T00:00:00"),o=new Date,s=new Date(o);s.setDate(s.getDate()+1);const n=t.toDateString()===o.toDateString(),a=t.toDateString()===s.toDateString(),i=String(t.getDate()).padStart(2,"0"),c=String(t.getMonth()+1).padStart(2,"0"),m=t.getFullYear();return n?`Hoje (${i}/${c}/${m})`:a?`Amanhã (${i}/${c}/${m})`:`${i}/${c}/${m}`},D=e=>{if(!e)return"";const[t,o]=e.split(":");return`${t}:${o}`},V=e=>{const t=e.color||"#ff8e4d",o=D(e.startTime),s=D(e.endTime),n=o&&s?`${o} - ${s}`:o||"Sem horário";return`
    ...
    `},q=e=>{const t=T,o=document.getElementById("view-event-title"),s=document.getElementById("view-event-description"),n=document.getElementById("view-event-start-time"),a=document.getElementById("view-event-end-time"),i=document.getElementById("view-event-date"),c=document.getElementById("view-event-id"),m=document.getElementById("view-event-color");o&&(o.textContent=e.title||"Sem título"),s&&(s.textContent=e.description||"-"),n&&(n.textContent=D(e.startTime)||"-"),a&&(a.textContent=D(e.endTime)||"-"),i&&(i.textContent=U(e.date)),c&&(c.value=e.id||""),m&&(m.value=e.color||"#ff8e4d"),t.classList.remove("hidden")},P=()=>{T.classList.add("hidden")},Te=(e,t)=>{if(e.length===0)return;const o=[...e].sort((r,x)=>{const h=r.startTime||"00:00",B=x.startTime||"00:00";return h.localeCompare(B)}),s=U(t),[n,a,i]=s.split("-").map(Number),c=new Date(n,a-1,i),m=c.toLocaleDateString("pt-BR",{weekday:"long"}),u=c.toLocaleDateString("pt-BR",{day:"numeric",month:"long",year:"numeric"}),v=`
