import"./index-BI9FPup0.js";document.addEventListener("DOMContentLoaded",()=>{const g=document.getElementById("app");g.innerHTML=`
  ...
  `;const l=document.getElementById("login-form"),c=document.getElementById("username"),o=document.getElementById("password"),y=document.getElementById("toggle-password"),a=document.getElementById("eye-icon"),n=document.getElementById("error-message"),f=document.getElementById("error-text"),m=document.getElementById("login-btn"),u=document.getElementById("login-btn-text"),p=document.getElementById("login-btn-spinner");y.addEventListener("click",()=>{const e=o.getAttribute("type")==="password"?"text":"password";o.setAttribute("type",e),e==="text"?(a.classList.remove("fa-eye"),a.classList.add("fa-eye-slash")):(a.classList.remove("fa-eye-slash"),a.classList.add("fa-eye"))});const i=e=>{f.textContent=e,n.classList.remove("hidden"),setTimeout(()=>{n.classList.add("hidden")},5e3)},d=e=>{e?(m.disabled=!0,u.textContent="Entrando...",p.classList.remove("hidden")):(m.disabled=!1,u.textContent="Entrar",p.classList.add("hidden"))},b=async(e,s)=>{await new Promise(t=>setTimeout(t,1e3));const r={ygor:"491583"};return r[e]&&r[e]===s?{success:!0}:{success:!1,message:"Usuário ou senha incorretos"}};l.addEventListener("submit",async e=>{e.preventDefault();const s=c.value.trim(),r=o.value;if(!s||!r){i("Por favor, preencha todos os campos");return}d(!0),n.classList.add("hidden");try{const t=await b(s,r);t.success?(sessionStorage.setItem("authenticated","true"),sessionStorage.setItem("username",s),window.location.href="/home.html"):(i(t.message||"Usuário ou senha incorretos"),d(!1))}catch(t){console.error("Erro na autenticação:",t),i("Erro ao fazer login. Tente novamente."),d(!1)}}),c.focus(),o.addEventListener("keypress",e=>{e.key==="Enter"&&l.dispatchEvent(new Event("submit"))})});
