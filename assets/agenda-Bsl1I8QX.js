...
⚠️ ${t.errors} eventos tiveram erros ao deletar.`:"")),window.location.reload()}catch(t){console.error("Erro na limpeza:",t),await window.calendar.showAlert("Erro na Limpeza",`❌ Erro na limpeza: ${t.message}`)}},console.log("💡 Para importar o Plano Anual, execute: importarPlanoAnual()"),console.log("💡 Para apagar todos os eventos, execute: limparTodosEventos()"));document.addEventListener("DOMContentLoaded",async()=>{if(!(sessionStorage.getItem("authenticated")==="true")){window.location.href="/index.html";return}const e=sessionStorage.getItem("username")||"Usuário",t=new N(e,"agenda"),o=document.getElementById("app");o.innerHTML=`
    ${t.render()}
