import"./index-BI9FPup0.js";import{S as Ye,L as Ue,G as Ve}from"./Sidebar-ByY7M0vv.js";document.addEventListener("DOMContentLoaded",async()=>{if(!(sessionStorage.getItem("authenticated")==="true")){window.location.href="/index.html";return}const Q=sessionStorage.getItem("username")||"Usuário",R=new Ye(Q,"home"),ke=document.getElementById("app");ke.innerHTML=`
    ${R.render()}
    <div class="main-content">
      ${R.renderTopBar(`Bem-vindo, ${Q}!`,"Resumo dos seus eventos")}
  ...
  `;const W=document.getElementById("loading-state"),Se=document.getElementById("error-state"),Le=document.getElementById("content"),Be=document.getElementById("next-events-carousel"),Y=document.getElementById("all-events-carousel");document.getElementById("logout-btn");const T=document.getElementById("event-view-modal"),M=document.getElementById("all-events-modal"),K=document.getElementById("all-events-modal-list"),Ce=document.getElementById("carousel-track"),_=document.getElementById("carousel-prev"),X=document.getElementById("carousel-next"),Ie=document.querySelectorAll(".carousel-dot"),E=document.getElementById("create-event-modal"),Z=document.getElementById("open-create-event-modal"),ee=document.getElementById("close-create-event-modal"),te=document.getElementById("cancel-create-event-btn"),H=document.getElementById("create-event-form"),oe=document.getElementById("create-event-date"),se="https://script.google.com/macros/s/AKfycbwurPvvvkvydBx2qN4WgZKMwsvXm31w3wMl_Qx9C70PAUT0hiUz-yHfaJgez2wrb7BTlQ/exec";let w;try{const e=new Ue({storageKey:"calendar-events"});if(se)try{const t=new Ve({scriptUrl:se});await t.initialize(),w=t}catch(t){console.error("❌ Erro ao conectar Google Apps Script:",t),w=e}await w.initialize()}catch(e){console.error("❌ Erro ao inicializar armazenamento:",e),ve("Erro ao conectar com o armazenamento de dados");return}const U=e=>{const t=new Date(e+"T00:00:00"),o=new Date,s=new Date(o);s.setDate(s.getDate()+1);const a=t.toDateString()===o.toDateString(),n=t.toDateString()===s.toDateString(),i=String(t.getDate()).padStart(2,"0"),c=String(t.getMonth()+1).padStart(2,"0"),m=t.getFullYear();return a?`Hoje (${i}/${c}/${m})`:n?`Amanhã (${i}/${c}/${m})`:`${i}/${c}/${m}`},D=e=>{if(!e)return"";const[t,o]=e.split(":");return`${t}:${o}`},V=e=>{const t=e.color||"#ff8e4d",o=D(e.startTime),s=D(e.endTime),a=o&&s?`${o} - ${s}`:o||"Sem horário";return`
    ...
    `},q=e=>{const t=T,o=document.getElementById("view-event-title"),s=document.getElementById("view-event-description"),a=document.getElementById("view-event-start-time"),n=document.getElementById("view-event-end-time"),i=document.getElementById("view-event-date"),c=document.getElementById("view-event-id"),m=document.getElementById("view-event-color");o&&(o.textContent=e.title||"Sem título"),s&&(s.textContent=e.description||"-"),a&&(a.textContent=D(e.startTime)||"-"),n&&(n.textContent=D(e.endTime)||"-"),i&&(i.textContent=U(e.date)),c&&(c.value=e.id||""),m&&(m.value=e.color||"#ff8e4d"),t.classList.remove("hidden")},P=()=>{T.classList.add("hidden")},Te=(e,t)=>{if(e.length===0)return;const o=[...e].sort((r,x)=>{const h=r.startTime||"00:00",L=x.startTime||"00:00";return h.localeCompare(L)}),s=U(t),[a,n,i]=s.split("-").map(Number),c=new Date(a,n-1,i),m=c.toLocaleDateString("pt-BR",{weekday:"long"}),u=c.toLocaleDateString("pt-BR",{day:"numeric",month:"long",year:"numeric"}),v=`
      <div id="events-list-modal" class="event-modal fixed inset-0 bg-black/70 flex items-center justify-center z-50">
        <div class="event-modal-content bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl shadow-2xl p-6 max-w-md w-full mx-4 max-h-[80vh] overflow-y-auto">
          ...
          <div class="space-y-2">
            ${o.map(r=>{const x=r.startTime&&r.endTime?`${D(r.startTime)} - ${D(r.endTime)}`:r.startTime?`${D(r.startTime)}`:"Horário não definido";return r.color,`
                <div 
                  class="event-list-item p-3 rounded-lg transition-colors hover:bg-[#222222] border-l-4"
                  style="border-left-color: ${r.color}; background-color: ${r.color}15;"
                  data-event-id="${r.id}"
                >
                  <div class="flex items-start justify-between gap-2">
                    ...
                    >
                      ...
                      <h4 class="text-sm font-semibold text-white mb-1">${r.title||"Sem título"}</h4>
                      ${r.description?`<p class="text-xs text-gray-400 line-clamp-2">${r.description}</p>`:""}
                    </div>
                  </div>
                </div>
              `}).join("")}
    ...
    `,l=document.getElementById("events-list-modal");l&&l.remove(),document.body.insertAdjacentHTML("beforeend",v);const d=document.getElementById("events-list-modal"),f=document.getElementById("close-events-list-modal"),b=document.getElementById("close-events-list-modal-btn"),y=()=>{d&&d.remove()};f&&f.addEventListener("click",y),b&&b.addEventListener("click",y),d&&d.addEventListener("click",r=>{r.target===d&&y()}),d.querySelectorAll('[data-action="view-event"]').forEach(r=>{r.addEventListener("click",()=>{const x=r.getAttribute("data-event-id"),h=o.find(L=>L.id===x);h&&(y(),q(h))})})},Me=e=>{K.innerHTML=e.length>0?e.map(V).join(""):'<p class="text-gray-400 text-center py-4">Nenhum evento encontrado</p>',M.classList.remove("hidden"),K.querySelectorAll(".event-item-row").forEach(t=>{t.addEventListener("click",()=>{const o=t.getAttribute("data-event-id"),s=e.find(a=>a.id===o);s&&($(),q(s))})})},$=()=>{M.classList.add("hidden")},ae=document.getElementById("close-view-modal"),ne=document.getElementById("close-view-modal-btn");ae&&ae.addEventListener("click",P),ne&&ne.addEventListener("click",P);const re=document.getElementById("close-all-events-modal"),ie=document.getElementById("close-all-events-modal-btn");re&&re.addEventListener("click",$),ie&&ie.addEventListener("click",$),T.addEventListener("click",e=>{e.target===T&&P()}),M.addEventListener("click",e=>{e.target===M&&$()}),document.addEventListener("keydown",e=>{e.key==="Escape"&&(T.classList.contains("hidden")||P(),M.classList.contains("hidden")||$(),E&&!E.classList.contains("hidden")&&F())});const $e=()=>{if(E){const e=new Date,t=e.getFullYear(),o=String(e.getMonth()+1).padStart(2,"0"),s=String(e.getDate()).padStart(2,"0"),a=`${t}-${o}-${s}`;oe&&(oe.value=a);const n=document.querySelector('#create-event-modal .color-option[data-color="#ff8e4d"]');n&&(document.querySelectorAll("#create-event-modal .color-option").forEach(i=>{i.classList.remove("border-primary-500","border-4"),i.classList.add("border-gray-300","border-2")}),n.classList.remove("border-gray-300","border-2"),n.classList.add("border-primary-500","border-4")),E.classList.remove("hidden")}},F=()=>{if(E&&(E.classList.add("hidden"),H)){H.reset();const e=document.getElementById("create-event-color");e&&(e.value="#ff8e4d"),document.querySelectorAll("#create-event-modal .color-option").forEach(o=>{o.classList.remove("border-primary-500","border-4"),o.classList.add("border-gray-300","border-2")});const t=document.getElementById("create-custom-color-container");t&&t.classList.add("hidden")}},Ae=()=>{const e=document.getElementById("create-event-start-time"),t=document.getElementById("create-event-end-time");e&&(e.addEventListener("click",function(){if(this.showPicker&&typeof this.showPicker=="function")try{this.showPicker()}catch{this.focus()}}),e.style.cursor="pointer"),t&&(t.addEventListener("click",function(){if(this.showPicker&&typeof this.showPicker=="function")try{this.showPicker()}catch{this.focus()}}),t.style.cursor="pointer")};Z&&Z.addEventListener("click",()=>{$e(),setTimeout(()=>{Ae()},100)}),ee&&ee.addEventListener("click",F),te&&te.addEventListener("click",F),E&&E.addEventListener("click",e=>{e.target===E&&F()}),document.querySelectorAll("#create-event-modal .color-option").forEach(e=>{e.addEventListener("click",()=>{const t=e.getAttribute("data-color"),o=document.getElementById("create-event-color");o&&(o.value=t),document.querySelectorAll("#create-event-modal .color-option").forEach(a=>{a.classList.remove("border-primary-500","border-4"),a.classList.add("border-gray-300","border-2")}),e.classList.remove("border-gray-300","border-2"),e.classList.add("border-primary-500","border-4");const s=document.getElementById("create-custom-color-container");s&&s.classList.add("hidden")})});const de=document.querySelector('#create-event-modal [data-action="open-custom-color"]'),le=document.getElementById("create-event-color-custom"),ce=document.getElementById("create-custom-color-container");de&&de.addEventListener("click",()=>{ce&&ce.classList.toggle("hidden")}),le&&le.addEventListener("change",e=>{const t=document.getElementById("create-event-color");t&&(t.value=e.target.value),document.querySelectorAll("#create-event-modal .color-option").forEach(o=>{o.classList.remove("border-primary-500","border-4"),o.classList.add("border-gray-300","border-2")})});const je=()=>Date.now().toString(36)+Math.random().toString(36).substr(2);H&&H.addEventListener("submit",async e=>{var n;e.preventDefault();const t=document.getElementById("submit-create-event-btn"),o=document.getElementById("submit-create-btn-text"),s=document.getElementById("submit-create-btn-spinner"),a=document.getElementById("create-save-status");t&&(t.disabled=!0,o&&o.classList.add("hidden"),s&&s.classList.remove("hidden")),a&&(a.classList.remove("hidden"),a.textContent="💾 Salvando evento...",a.className="mt-2 text-sm text-center text-blue-600 font-medium");try{const i=document.getElementById("create-event-title").value,c=document.getElementById("create-event-description").value,m=document.getElementById("create-event-date").value,u=document.getElementById("create-event-start-time").value,v=document.getElementById("create-event-end-time").value,l=document.getElementById("create-event-color").value,d=m,f=(l||"#3b82f6").toLowerCase()==="#3b82f6",b=je();if(f)if(console.log("🔵 Evento azul detectado - salvando na aba Ações"),w&&typeof w.saveAction=="function"){const r=new Date,x=r.getFullYear(),h=String(r.getMonth()+1).padStart(2,"0"),L=String(r.getDate()).padStart(2,"0"),p=`${x}-${h}-${L}`,g=u||"";try{await w.saveAction({id:b,title:i.trim()||"",description:c.trim()||"",createdAt:new Date().toISOString(),createdDate:p,createdTime:g,finishedDate:"",finishedTime:""}),console.log("✅ Ação salva com sucesso na aba Ações")}catch(k){throw console.error("❌ Erro ao salvar ação:",k),new Error(`Erro ao salvar ação: ${k.message}`)}}else{const r=w?`Serviço de armazenamento (${((n=w.constructor)==null?void 0:n.name)||"desconhecido"}) não suporta salvar ações. Use GoogleAppsScriptService.`:"Serviço de armazenamento não disponível";throw console.error("❌",r),new Error(r)}const y={id:b,title:i.trim(),description:c.trim(),date:d,startTime:u||"",endTime:v||"",color:l||"#ff8e4d",createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};await w.saveEvent(y),a&&(a.textContent="✅ Evento criado com sucesso!",a.className="mt-2 text-sm text-center text-green-600 font-medium"),setTimeout(()=>{window.location.reload()},1e3)}catch(i){console.error("Erro ao salvar evento:",i),a&&(a.textContent="❌ Erro ao salvar evento. Tente novamente.",a.className="mt-2 text-sm text-center text-red-600 font-medium"),t&&(t.disabled=!1,o&&o.classList.remove("hidden"),s&&s.classList.add("hidden"))}});const me=e=>e.sort((t,o)=>{const s=new Date(t.date+"T"+(t.startTime||"00:00")),a=new Date(o.date+"T"+(o.startTime||"00:00"));return s-a}),He=(e,t)=>{const o=new Date;o.setHours(0,0,0,0);const s=new Date;switch(t){case"today":return e.filter(u=>new Date(u.date+"T00:00:00").toDateString()===o.toDateString());case"week":const a=o.getDay();let n,i;if(a===0||a===6){const u=a===0?1:2;n=new Date(o),n.setDate(o.getDate()+u),n.setHours(0,0,0,0),i=new Date(n),i.setDate(n.getDate()+4),i.setHours(23,59,59,999)}else{n=new Date(o),n.setHours(0,0,0,0);const u=5-a;i=new Date(o),i.setDate(o.getDate()+u),i.setHours(23,59,59,999)}return e.filter(u=>{const v=new Date(u.date+"T00:00:00"),l=v.getDay(),d=l>=1&&l<=5;return v>=n&&v<=i&&d});case"month":const c=new Date(o.getFullYear(),o.getMonth(),1),m=new Date(o.getFullYear(),o.getMonth()+1,0);return m.setHours(23,59,59,999),e.filter(u=>{const v=new Date(u.date+"T00:00:00");return v>=c&&v<=m});case"upcoming":return e.filter(u=>new Date(u.date+"T"+(u.startTime||"00:00"))>=s);case"all":default:return e}},ue=(e,t=null)=>{const o=new Date,s=e.filter(n=>new Date(n.date+"T"+(n.startTime||"00:00"))>=o),a=me(s);return t?a.slice(0,t):a},qe=e=>{const t=new Date;t.setHours(0,0,0,0);const o=e.filter(p=>new Date(p.date+"T00:00:00").toDateString()===t.toDateString()),s=document.getElementById("stats-today");s&&(s.textContent=o.length);const a=ue(e),n=a.length>0?a[0]:null,i=document.getElementById("stats-next-title"),c=document.getElementById("stats-next-time"),m=document.getElementById("stats-next-title-mobile"),u=document.getElementById("stats-next-time-mobile"),v=(p,g)=>{if(n&&p&&g){p.textContent=n.title||"Sem título";const k=U(n.date),I=D(n.startTime),De=D(n.endTime);I&&De?g.textContent=`${k} • ${I} - ${De}`:I?g.textContent=`${k} • ${I}`:g.textContent=k}else p&&g&&(p.textContent="Nenhum",g.textContent="-")};v(i,c),v(m,u);const l=t.getDay();let d,f;if(l===0||l===6){const p=l===0?1:2;d=new Date(t),d.setDate(t.getDate()+p),d.setHours(0,0,0,0),f=new Date(d),f.setDate(d.getDate()+4),f.setHours(23,59,59,999)}else{d=new Date(t),d.setHours(0,0,0,0);const p=5-l;f=new Date(t),f.setDate(t.getDate()+p),f.setHours(23,59,59,999)}const b=e.filter(p=>{const g=new Date(p.date+"T00:00:00"),k=g.getDay(),I=k>=1&&k<=5;return g>=d&&g<=f&&I}),y=document.getElementById("stats-week");y&&(y.textContent=b.length);const r=new Date(t.getFullYear(),t.getMonth(),1),x=new Date(t.getFullYear(),t.getMonth()+1,0);x.setHours(23,59,59,999);const h=e.filter(p=>{const g=new Date(p.date+"T00:00:00");return g>=r&&g<=x}),L=document.getElementById("stats-month");L&&(L.textContent=h.length)},ve=e=>{W.classList.add("hidden"),Se.classList.remove("hidden"),document.getElementById("error-message").textContent=e};let A="all",j="";const Pe=(e,t)=>{if(!t||t.trim()==="")return e;const o=t.toLowerCase().trim();return e.filter(s=>{const a=(s.title||"").toLowerCase(),n=(s.description||"").toLowerCase();return a.includes(o)||n.includes(o)})},xe=async(e=A,t=j)=>{try{const o=await w.loadEvents(),s=me([...o]);let a=He(s,e);t&&t.trim()!==""&&(a=Pe(a,t)),pe=a;const n=ue(s,3);Be.innerHTML=n.length>0?n.map(V).join(""):'<p class="text-gray-400 text-center py-4">Nenhum evento próximo</p>';const i=a.slice(0,3);Y.innerHTML=i.length>0?i.map(V).join(""):'<p class="text-gray-400 text-center py-4">Nenhum evento encontrado</p>';const c=document.getElementById("view-all-events-btn");if(c)if(a.length>3){c.classList.remove("hidden");const m=document.getElementById("view-all-text");m&&(m.textContent=`Ver todos os ${a.length} eventos`)}else c.classList.add("hidden");document.querySelectorAll(".event-item-row").forEach(m=>{m.addEventListener("click",()=>{const u=m.getAttribute("data-event-id"),v=s.find(l=>l.id===u);v&&q(v)})}),qe(s),ze(s),W.classList.add("hidden"),Le.classList.remove("hidden")}catch(o){console.error("Erro ao carregar eventos:",o),ve(o.message||"Erro desconhecido ao carregar eventos")}};R.init();let pe=[],B=new Date,ge=[];const Fe=e=>{const t=e.getFullYear(),o=String(e.getMonth()+1).padStart(2,"0"),s=String(e.getDate()).padStart(2,"0");return`${t}-${o}-${s}`},Ne=e=>ge.filter(t=>t.date===e),O=()=>{const e=document.getElementById("mini-calendar-grid"),t=document.getElementById("mini-calendar-month-year");if(!e||!t)return;const o=B.getFullYear(),s=B.getMonth(),a=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];t.textContent=`${a[s]} ${o}`;const n=new Date(o,s,1),i=new Date(o,s+1,0),c=new Date;c.setHours(0,0,0,0);const m=n.getDay(),u=i.getDate();e.querySelectorAll(".mini-cal-day, .mini-cal-day-empty").forEach(l=>l.remove());for(let l=0;l<m;l++){const d=document.createElement("div");d.className="mini-cal-day-empty",e.appendChild(d)}for(let l=1;l<=u;l++){const d=new Date(o,s,l),f=Fe(d),b=Ne(f),y=d.toDateString()===c.toDateString(),r=d.getMonth()===s,x=document.createElement("div");if(x.className=`mini-cal-day relative p-2 text-center rounded-lg cursor-pointer transition-all ${r?"":"opacity-30"} ${y?"bg-transparent border-2 border-red-500 text-red-400 font-bold":b.length>0?"bg-[#222222] hover:bg-[#2a2a2a] text-white":"bg-[#1a1a1a] hover:bg-[#222222] text-gray-300"}`,x.textContent=l,x.setAttribute("data-date",f),x.setAttribute("data-day",l),b.length>0&&r){const h=document.createElement("div");h.className="absolute bottom-1 left-1/2 transform -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-white",x.appendChild(h)}x.addEventListener("click",()=>{r&&b.length>0&&(b.length===1?q(b[0]):Te(b,d))}),e.appendChild(x)}},ze=e=>{ge=e,O()},G=async(e=A,t=j)=>{Y&&(Y.innerHTML=`
      ...
      `),await xe(e,t)},N=document.getElementById("search-events-input"),C=document.getElementById("clear-search-btn");if(N){let e;N.addEventListener("input",t=>{const o=t.target.value;j=o,C&&(o.trim()!==""?C.classList.remove("hidden"):C.classList.add("hidden")),clearTimeout(e),e=setTimeout(()=>{G(A,o)},300)})}C&&C.addEventListener("click",()=>{N&&(N.value="",j="",C.classList.add("hidden"),G(A,""))});const fe=document.querySelectorAll(".filter-quick-btn");fe.forEach(e=>{e.addEventListener("click",async()=>{const t=e.getAttribute("data-filter");fe.forEach(o=>o.classList.remove("active")),e.classList.add("active"),A=t,await G(t,j)})});const be=document.getElementById("view-all-events-btn");be&&be.addEventListener("click",()=>{Me(pe)});let S=0;const z=2,J=()=>{const e=document.querySelectorAll(".carousel-slide"),t=document.querySelectorAll(".carousel-dot");e.forEach((o,s)=>{s===S?o.classList.add("active"):o.classList.remove("active")}),t.forEach((o,s)=>{s===S?o.classList.add("active"):o.classList.remove("active")}),Ce.style.transform=`translateX(-${S*100}%)`},Re=e=>{e>=0&&e<z&&(S=e,J())},ye=()=>{S=(S+1)%z,J()},he=()=>{S=(S-1+z)%z,J()};X&&X.addEventListener("click",ye),_&&_.addEventListener("click",he),Ie.forEach((e,t)=>{e.addEventListener("click",()=>Re(t))}),document.addEventListener("keydown",e=>{e.key==="ArrowLeft"&&he(),e.key==="ArrowRight"&&ye()});const we=document.getElementById("mini-calendar-prev"),Ee=document.getElementById("mini-calendar-next");we&&we.addEventListener("click",()=>{B.setMonth(B.getMonth()-1),O()}),Ee&&Ee.addEventListener("click",()=>{B.setMonth(B.getMonth()+1),O()}),await xe()});
