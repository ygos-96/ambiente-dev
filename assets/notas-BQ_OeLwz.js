import"./index-DPlvyXvB.js";import{S as ge}from"./Sidebar-38HLq7tH.js";import{P as be}from"./PersonalDataService-CClDzJ6y.js";import{G as ve}from"./GoogleAppsScriptService-CQm_j4Wp.js";const xe="/ambiente-dev/";document.addEventListener("DOMContentLoaded",async()=>{if(!(sessionStorage.getItem("authenticated")==="true")){window.location.href=`${xe}index.html`;return}const Y=sessionStorage.getItem("username")||"Usuário",H=new ge(Y,"notas"),o=new be;await o.initialize();const ee="https://script.google.com/macros/s/AKfycbwBlZclk0FFoOwwhXKUU2xjHC7VtvCNZaJeOb7Wad_qf33AyhmzdirL4qlQR87VTHQg7Q/exec";let u=null;try{u=new ve({scriptUrl:ee}),await u.initialize()}catch(a){console.warn("Planilha (Notas) não disponível:",a)}const te=document.getElementById("app");te.innerHTML=`
    ${H.render()}
  ...
  `,H.init();const ae=document.getElementById("tab-notas"),ie=document.getElementById("tab-listas"),se=document.getElementById("tab-links"),R=document.querySelectorAll(".tab-btn"),U=document.getElementById("notas-list"),x=document.getElementById("listas-container"),W=document.getElementById("links-list"),Q=document.getElementById("nota-titulo"),ne=document.getElementById("add-nota"),w=document.getElementById("nova-lista-nome"),A=document.getElementById("criar-lista-btn"),_=document.getElementById("link-titulo"),J=document.getElementById("link-url"),oe=document.getElementById("add-link"),q=document.getElementById("nota-modal"),j=document.getElementById("nota-modal-titulo"),K=document.getElementById("nota-modal-conteudo"),le=document.getElementById("nota-modal-close"),N=document.getElementById("nota-modal-save"),de=document.getElementById("nota-modal-delete"),m=document.getElementById("delete-confirm-modal"),re=document.getElementById("delete-cancel-btn"),S=document.getElementById("delete-confirm-btn"),ce=document.getElementById("toast-container"),f=document.getElementById("list-type-modal"),V=document.getElementById("list-type-cancel"),ue=document.querySelectorAll(".list-type-btn");let $=null,k=null,T=null;function p(a,e="success"){const t=document.createElement("div");let i,s,n;e==="success"?(i="bg-green-600",s="border-green-500",n="fa-check-circle"):e==="error"?(i="bg-red-700",s="border-red-500",n="fa-exclamation-triangle"):(i="bg-blue-600",s="border-blue-500",n="fa-info-circle"),t.className=`${i} border-2 ${s} text-white px-4 py-3 rounded-lg shadow-2xl flex items-center gap-3 animate-slide-in-right font-medium z-[200] min-w-[300px] max-w-md`,t.style.zIndex="200",t.innerHTML=`
      <i class="fas ${n} text-lg"></i>
      <span class="flex-1">${r(a)}</span>
    `,ce.appendChild(t),setTimeout(()=>{t.classList.add("animate-slide-out-right"),setTimeout(()=>t.remove(),300)},e==="error"?5e3:3e3)}function z(a,e,t,i){e?(a.disabled=!0,t.classList.add("hidden"),i.classList.remove("hidden")):(a.disabled=!1,t.classList.remove("hidden"),i.classList.add("hidden"))}function G(a){R.forEach(e=>{const t=e.dataset.tab===a;e.classList.toggle("bg-blue-600",t),e.classList.toggle("text-white",t),e.classList.toggle("text-gray-400",!t),e.classList.toggle("hover:bg-[#222222]",!t)}),ae.classList.toggle("hidden",a!=="notas"),ie.classList.toggle("hidden",a!=="listas"),se.classList.toggle("hidden",a!=="links"),a==="listas"&&g(),a==="links"&&D()}R.forEach(a=>{a.addEventListener("click",()=>G(a.dataset.tab))});function r(a){const e=document.createElement("div");return e.textContent=a||"",e.innerHTML}function Z(a){const e=(a||"").toLowerCase().trim();if(!e)return{difficulty:"Fácil",estimatedTimeMinutes:5,estimatedTimeLabel:"5 min"};const t=e.split(/\s+/).filter(Boolean),i=["oi","ok","confirmar","avisar","responder","dar retorno","mandar mensagem"],s=["ligar","enviar","pagar","comprar","agendar","checar","verificar","postar","publicar","organizar"],n=["revisar","ajustar","montar","escrever","planejar"],l=["editar vídeo","editar video","edição de vídeo","edicao de video","refatorar","implementar","corrigir bug","pesquisar","documentar","integrar","configurar","deploy","estudar","aprender"],c=i.some(I=>e.includes(I)),b=s.filter(I=>e.includes(I)).length,d=n.filter(I=>e.includes(I)).length,L=l.filter(I=>e.includes(I)).length,F=(e.includes("editar")||e.includes("edição")||e.includes("edicao"))&&(e.includes("vídeo")||e.includes("video"));let v=0;t.length<=3?v+=0:t.length<=7?v+=1:t.length<=12?v+=2:v+=3,v+=d,v+=L*2,v-=b;let C="Médio",y=30;c&&t.length<=4&&!F?(C="Fácil",y=3):v<=0?(C="Fácil",y=8):v>=4&&(C="Difícil",y=60),F?(C=t.length>5?"Difícil":"Médio",y=t.length>5?90:45):v>=6&&(y=90);const O=Math.floor(y/60),X=y%60,pe=O>0?X>0?`${O}h ${X}min`:`${O}h`:`${y} min`;return{difficulty:C,estimatedTimeMinutes:y,estimatedTimeLabel:pe}}function me(a){return a==="Difícil"?{label:"Difícil",className:"bg-red-900/40 text-red-300 border border-red-700/50"}:a==="Fácil"?{label:"Fácil",className:"bg-green-900/40 text-green-300 border border-green-700/50"}:{label:"Médio",className:"bg-yellow-900/40 text-yellow-300 border border-yellow-700/50"}}function B(){const a=o.getNotes();U.innerHTML=a.length===0?'<p class="text-gray-400 text-sm">Nenhuma nota. Clique em "Nova nota" para criar.</p>':a.map(e=>`
        ...
        `).join("")}U.addEventListener("click",a=>{const e=a.target.closest("[data-note-id]");e&&M(e.dataset.noteId)});function M(a){$=a;let t=o.getNotes().find(i=>i.id===a);if(!t&&h){const s=o.getLists().find(n=>n.id===h);if(s){const n=s.items.find(l=>l.id===E&&l.type==="note");n&&(t=o.saveNote({id:n.id,title:n.title||"",content:n.content||""}))}}t?(j.value=t.title||"",K.value=t.content||""):(j.value="",K.value=""),q.classList.remove("hidden"),q.classList.add("flex"),j.focus()}function P(){q.classList.add("hidden"),q.classList.remove("flex"),$=null}ne.addEventListener("click",()=>{const a=Q.value.trim(),e=o.saveNote({title:a||"Nova nota",content:""});Q.value="",B(),M(e.id),u&&u.saveNote({id:e.id,title:e.title||"",content:e.content||""}).then(()=>{p("Nota registrada na planilha!","success")}).catch(t=>{console.warn("Falha ao enviar nota para a planilha:",t)})}),le.addEventListener("click",P),N.addEventListener("click",async()=>{if($==null)return;const a=N.querySelector(".save-text"),e=N.querySelector(".save-spinner");z(N,!0,a,e);try{const t=o.saveNote({id:$,title:j.value.trim(),content:K.value.trim()});if(h&&E){const i=o.getLists(),s=i.find(n=>n.id===h);if(s){const n=s.items.find(l=>l.id===E);n&&n.type==="note"&&(n.title=t.title,n.content=t.content,o.saveLists(i))}}if(u)try{await u.saveNote({id:t.id,title:t.title||"",content:t.content||""}),p("Nota salva e atualizada na planilha!","success")}catch(i){console.warn("Falha ao atualizar nota na planilha:",i),p("Nota salva localmente, mas não foi possível atualizar na planilha","error")}else p("Nota salva!","success");B(),h&&g(),P(),E=null,h=null}finally{z(N,!1,a,e)}}),de.addEventListener("click",()=>{$!=null&&(k=$,m.classList.remove("hidden"),m.classList.add("flex"))}),re.addEventListener("click",()=>{m.classList.add("hidden"),m.classList.remove("flex"),k=null}),m.addEventListener("click",a=>{a.target===m&&(m.classList.add("hidden"),m.classList.remove("flex"),k=null)}),S.addEventListener("click",async()=>{if(k==null)return;const a=S.querySelector(".delete-text"),e=S.querySelector(".delete-spinner");z(S,!0,a,e);try{if(u)try{await u.deleteNote(k)}catch(t){console.warn("Falha ao remover nota da planilha:",t)}o.deleteNote(k),p("Nota excluída com sucesso!","success"),B(),P(),m.classList.add("hidden"),m.classList.remove("flex"),k=null}catch{p("Erro ao excluir nota","error")}finally{z(S,!1,a,e)}});let E=null,h=null;function g(){const a=o.getLists();x.innerHTML=a.map(e=>{const t=e.items||[];return`
        <div class="bg-[#1a1a1a] rounded-lg border border-[#2a2a2a] p-4 mb-4">
          ...
          <ul class="space-y-2 mb-3" data-list-id="${e.id}">
            ${t.length===0?'<li class="text-gray-500 text-sm">Nenhum item.</li>':t.map(i=>{const n=e.listType==="tasks";if(i.type==="note")return`
                ...
                `;{const l=Z(i.text||""),c=l.difficulty,b=l.estimatedTimeLabel,d=me(c);return`
